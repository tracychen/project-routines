...
7. **Bump it in both places**: `plugins/<name>/.claude-plugin/plugin.json` *and* the entry in `.claude-plugin/marketplace.json`. The desktop "Update available" badge reads `plugin.json#version` and falls back to `marketplace.json#plugins[].version`, and which one it can see depends on the entry's source shape — keeping them in sync makes update surfacing reliable either way. Note the badge reads these from the installer's local **marketplace clone**, so a published bump only appears after that marketplace is refreshed.
